...
.PARAMETER DbName
    Name of the source database to migrate. Required.
...
function Remove-ActiveConnections {
    ...
    & $psqlExe -U $PgUser -p $PgPort -d postgres -c `
        "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '$Database' AND pid <> pg_backend_pid();" 2>$null | Out-Null
}
...
if ($LASTEXITCODE -ne 0) {
    throw "pg_dump failed. Check PostgreSQL logs."
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Failed to rename database. Check for remaining active connections."
}
